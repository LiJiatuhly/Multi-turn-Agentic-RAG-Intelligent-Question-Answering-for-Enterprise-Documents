...
# └──────────────────────────────────────────────────────────────
#
# ★★ 本文件已把每个字段标注成【写】→【读】→【🔢 真实长什么样】三段式 ★★
#   【写】   = 哪个函数把值放进这个字段（函数名后括号注明来自 nodes.py / edges.py）
#   【读】   = 哪个函数把值取出来用
#   🔢      = 这个字段【真实装着什么】—— 有值时长什么样、空的时候长什么样
#            （只知道"它是个字符串集合"没用，得看到里面真的装着 "parent::公司规章制度_p0"）
# 想深入看某个字段怎么流转，照着【写】【读】里的函数名去 nodes.py / edges.py 找即可。
...
def accumulate_or_reset(existing: List[dict], new: List[dict]) -> List[dict]:
    ...
    #    `new and ...` 短路：new 是空 → 直接跳到 return existing + new（等于什么也没加）。
    #
    # 📎 隐形合同：'__reset__' 这个字符串键，必须和 nodes.summarize_history 里
...
class State(MessagesState):   # 继承 MessagesState → 自动带 messages 字段
    ...
    # ── messages（继承自 MessagesState，没写在下面但真实存在）──────────
    ...
    #   【读】summarize_history、rewrite_query、aggregate_answers
    #
    #   🔢 有值（一轮问答结束后）：
    #     [HumanMessage(content="上海出差住宿和交通能报多少"),
    #      AIMessage(content="上海出差：住宿费每天上限 500 元；交通费按高铁二等座据实报销。")]
    #      ↑ 注意这两条【都没有 name 属性】—— 这是"真实对话"的标志
    #
    #   🔢 中间态（并行子图跑完、aggregate_answers 还没清理时，是一锅粥）：
    #     [HumanMessage("上海出差住宿和交通能报多少"),                          ← 无 name ✅ 真实对话
    #      AIMessage(content="", tool_calls=[{...}], name="agent_response"),   ← 有 name ❌ 子图流回的
    #      ToolMessage(content="Parent ID: 公司规章制度_p0\\n…",
    #                  name="search_child_chunks"),                            ← 有 name ❌ 子图流回的
    #      AIMessage("每天上限 500 元。", name="agent_response")]                ← 有 name ❌ 子图流回的
    #
    #   🔢 空态：[]（全新会话，用户还没说话）
    #
    ...
    #   【读】route_after_rewrite（edges.py）：True→并行发 Agent；False→去 request_clarification
    #
    ...
    #   【写】summarize_history
    #   【读】rewrite_query（改写时当上下文用）；summarize_history 自己也读旧摘要来合并成新摘要
    #
    #   🔢 有值（聊了好几轮之后）：
    ...
    #   【读】rewrite_query（下一轮和 pendingQuery 一起拼成完整检索问题）—— 同样是它自己写自己读
    #
    #   🔢 有值：["差旅", "住宿费"]     ← 用户被追问了两次，各补充了一句
    #   🔢 空态：[]                    ← 问清楚了，清空
    #
    #   ⚠️ 注意它【没有 reducer】—— 因为只有 rewrite_query 一个人写它，不存在并行冲突。
    #      每次是 rewrite_query 自己算好"旧列表 + 新输入"再整个覆盖进去。

    rewrittenQuestions: List[str] = []
    #   改写后的子问题列表（最多 3 个，用来派并行 Agent）。
    #   【写】rewrite_query（清晰时=改写/拆分后的 1~3 个子问题；不清晰时=[]）
    #   【读】route_after_rewrite（edges.py）：遍历它，给每个子问题发一个 Send 启动并行 Agent
    #
    ...
    #
    # ── 全项目四张纸条对照（同一个语法，纸条内容不同）─────────────────
    #   Annotated[List[dict], accumulate_or_reset]  → 并行答案：追加 / 一键清空
    #   Annotated[Set[str],   set_union]            → 已搜清单：求并集去重
    #   Annotated[List[str],  append_unique]        → 检索原文：追加 + 去重 + 保序
    #   Annotated[int,        operator.add]         → 两个计数器：新旧值相加
    #   ⚠️ 没贴纸条的字段（questionIsClear、conversation_summary…）= 直接覆盖，谁后写谁生效。
    #
    # ── 🎁 同一个语法，换个框架就换个用法（你学 FastAPI 时会再见到它）──────
    #   LangGraph : Annotated[List[dict], accumulate_or_reset]
    ...
    # ══════════════════════════════════════════════════════════════════════
    #
    #   【写】① summarize_history（nodes.py）：每轮开头发 [{'__reset__': True}] 清空上一轮
    ...
    #          所以 aggregate_answers 必须 sorted(key=lambda x: x["index"]) 重排。
...
class AgentState(MessagesState):   # 同样继承 → 自动带 messages 字段
    ...
    #   【写】① orchestrator：写入问题 + 模型回复 ② tools（ToolNode）：写入 ToolMessage 检索结果
    #        ③ compress_context：发 RemoveMessage 删掉除首条外的旧消息 ④ fallback_response：写兜底答案
    #   【读】orchestrator、route_after_orchestrator_call（edges.py）、should_compress_context、
    #        compress_context、fallback_response、collect_answer（以及辅助函数 _retrieval_contexts）
    #
    #   🔢 逐圈演变（真实轨迹，这个子图负责"住宿费"这个子问题）：
    #     刚被 Send 创建：  []                                       ← ⭐ 空白草稿纸
    #     orchestrator 第1圈后：
    #       [HumanMessage("上海出差住宿费报销上限是多少"),            ← 子问题
    #        AIMessage(content="",                                  ← ⚠️ 只调工具不说话 → content 是空的
    ...
    #       + AIMessage(content="每天上限 500 元。", tool_calls=[])   ← ⭐ tool_calls 空了 = 答完了
    #
    #   🔢 被 compress_context 压缩后：只剩 [HumanMessage("上海出差住宿费报销上限是多少")]
    #      ⚠️ 原文全被 RemoveMessage 烧光了，精华挪进了 context_summary。
    #      （所以才需要 retrieved_contexts 提前备份 —— 见 append_unique 的说明）
    #
    ...
    #   【读】orchestrator（第一轮作为问题发给模型）、compress_context、fallback_response、collect_answer
    #
    #   🔢 "上海出差住宿费报销上限是多少"     ← 0 号子图拿到的
    #   🔢 "上海出差交通费怎么报销"           ← 1 号子图拿到的（另一个独立实例）
    #   ⚠️ 它是被 Send("agent", {"question": ..., ...}) 塞进来的 ——
    #      不是别的节点写的，是【创建这个子图时就注入的初始值】。

    question_index: int = 0
    #   编号（汇总答案时按它排序）。
    #   【写】route_after_rewrite（edges.py）里的 Send 传入
    #   【读】collect_answer（放进 agent_answers 的 index 字段，供主图 aggregate_answers 排序）
    #
    ...
    #   【写】compress_context
    #   【读】orchestrator（作为已知背景注入模型）、should_compress_context（估 token 定要不要压）、
    ...
    #      ⚠️ 所以代码里全是 state.get("context_summary", "").strip()：
    ...
    # Annotated[Set[str], set_union] → 纸条上写 set_union，多次写入时求并集去重。
    #   【写】should_compress_context（把最近一轮模型调过的工具记成 "search::…"/"parent::…" 并入集合）
    #   【读】should_compress_context 自己 + compress_context（写进摘要末尾的"已执行清单"，提醒模型别重复搜）
    #
    #   🔢 有值（跑了两轮之后）：
    #     {
    #         "parent::公司规章制度_p0",     # parent:: 前缀 + 父块ID（调 retrieve_parent_chunks 记下的）
    #         "parent::员工手册_p3",
    #         "search::上海住宿费报销上限",   # search:: 前缀 + 搜索词（调 search_child_chunks 记下的）
    #         "search::出差补贴标准",
    #     }
    #   🔢 空态：set()（一次工具都还没调）
    #
    ...
    # Annotated[List[str], append_unique] → 纸条上写 append_unique，多次写入时追加去重。
    #   【写】should_compress_context（在 Command 里用 _retrieval_contexts(messages) 抽出有效检索原文）
    #   【读】collect_answer（塞进 agent_answers 的 "contexts" 字段，随答案一起冒泡到主图，供 RAGAS 评测）
    #
    #   🔢 有值：
    #     ["Parent ID: 公司规章制度_p0\\nFile Name: 规章制度.pdf\\nContent: 上海出差住宿费上限500元/天",
    #      "Parent ID: 公司规章制度_p1\\nFile Name: 规章制度.pdf\\nContent: 交通费按高铁二等座据实报销"]
    #   🔢 空态：[]（一次都没搜到有效内容，或工具全返回 NO_RELEVANT_CHUNKS 这类"暗号"被过滤掉了）
    #
    #   ⭐⭐ 这个字段【存在的唯一理由】，是为了抵消 compress_context 的副作用：
    #      compress_context 会用 RemoveMessage 把 messages 里的检索原文烧光，
    #      可 RAGAS 评测又需要"模型看到了哪些原文"。所以必须提前备份到这里。
    #
    #   📎 隐形合同：里面存的字符串格式（"Parent ID / File Name / Content"）
    #      来自 tools.py 拼字符串的地方，由 nodes._retrieval_contexts 反向解析。
    #      改了 tools.py 的格式或"暗号"词，这里就会存进一堆垃圾。
    retrieved_contexts: Annotated[List[str], append_unique] = []

    # 子图自己的 agent_answers：⚠️ 注意它【没有 Annotated】，是普通列表、无 reducer。
    # 因为子图里只有一个 collect_answer 节点写它，不存在并行写入，直接赋值就行。
    # 子图结束时，这里的值会"冒泡"到主图的 agent_answers（那个【有】reducer，会追加）。
    #   【写】collect_answer（本子图唯一写入点，所以不需要 reducer）
    #   【读】子图内无人读；子图结束时整体冒泡到主图 State.agent_answers（那个带 accumulate_or_reset 会追加）
    #
    #   🔢 有值（这个子图跑完了）：
    #     [{"index": 0,
    #       "question": "上海出差住宿费报销上限是多少",
    ...
    #      子图 AgentState.agent_answers ：无纸条 → 直接赋值   （单点写，不会冲突）
    #      主图 State.agent_answers      ：有纸条 → 追加合并   （N 个子图并行写，必须追加）
    ...
    #    orchestrator 里写的是 return {"iteration_count": 1}   ← "我这圈 +1"
    #    而不是                 return {"iteration_count": iteration + 1}  ← ❌ 不用你自己算
    #    框架看到纸条上的 operator.add，自动执行 operator.add(旧总数, 1)。
    #
    #   🔢 tool_call_count 逐圈演变（这个子图跑了 3 圈）：
    #     初始 0  →  第1圈 orchestrator 返回 {"tool_call_count": 1}  → 框架算 add(0, 1) = 1
    #             →  第2圈 返回 {"tool_call_count": 2}（这圈模型一次调了2个工具）→ add(1, 2) = 3
    #             →  第3圈 模型不调工具了，返回 {"tool_call_count": 0} → add(3, 0) = 3
    #   🔢 空态：0
    #   tool_call_count：【写】orchestrator（每轮 += 本轮工具调用数）
    #                    【读】route_after_orchestrator_call（edges.py）：超 MAX_TOOL_CALLS → 触发兜底
    ...
    #                    【读】route_after_orchestrator_call（edges.py）：超 MAX_ITERATIONS → 触发兜底
    #
    #   🎨 对照实验（把 operator.add 这张纸条撕掉会怎样）：
    #     ★ 有纸条 ★  逐圈的值: 1, 2, 3, 4, 5, 6, 7, 8  → 第 8 圈达到 MAX_ITERATIONS，刹车 ✅
    #     ☆ 无纸条 ☆  逐圈的值: 1, 1, 1, 1, 1, 1, 1, 1  → 每圈都被【覆盖】成 1
    #                                                     → 永远到不了 8 → 💸 无限循环烧钱 ❌
